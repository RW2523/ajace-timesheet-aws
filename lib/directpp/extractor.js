...
export async function directExtract({ input, fileName, month, year }) {
  const trace = [];
  const act = (agent, action, detail = "", model = null, ok = true, extra = null) =>
    trace.push({ agent, action, detail: String(detail).slice(0, 300), model, ok,
                 ...(extra || {}) });

  // ITEM 9 — stopwatch. Every OpenRouter call records its wall clock and the
  // token counts chatJson already returns and the caller used to discard. This
  // is pure observation: it reads no hours and changes no result.
  const t0All = Date.now();
  const calls = [];
  const timed = async (stage, model, messages, opts) => {
    const t = Date.now();
    const r = await chatJson(model, messages, opts);
    calls.push({ stage, model, ms: Date.now() - t,
  ...
  for (const model of CFG.ladder) {
    const { data, text, error } = await timed("extract", model,
    ...
    if (CFG.repair && !repaired && needsRepair(data, res)) {
      ...
      const fixed = await repairRound(model, system, pdf, images, extraText,
                                      text, data, res, month, year, act, timed);
  ...
  if (spread <= CFG.agreementTolerance &&
    ...
    await verifyRound(best, pdf, images, extraText, month, year, act, timed);
  }

  const timings = {
    total_ms: Date.now() - t0All,
    model_ms: calls.reduce((s, c) => s + c.ms, 0),
    calls,
  };
  act("DirectTiming", "measured",
      `${calls.length} model call(s), ${timings.model_ms}ms in models, ` +
      `${timings.total_ms}ms total`, null, true, { timings });

  return {
    employee: shapeEmployee(best, bestData, fileName, month, year),
    trace, timings, reason: null,
  };
}
...
function mapContract(data, month, year, model, act) {
  ...
  // OBSERVATION ONLY — which months the clipped-away entries belonged to.
  // The clip below is unchanged: entries outside the requested month are still
  // discarded, exactly as before, and nothing in this map feeds a total, the
  // accept gate, the confidence, or the review status. It exists so the UI can
  // say "this looks like a July document" instead of rendering an unexplained
  // empty calendar when someone reads a July timesheet into an August period.
  const offMonth = new Map();
  for (const ent of data.entries || []) {
    const d = normDate(ent.date, month, year);
    if (!d) continue;
    if (!d.startsWith(prefix)) {                            // clip to month
      const k = d.slice(0, 7);                              // YYYY-MM
      offMonth.set(k, (offMonth.get(k) || 0) + 1);
      continue;
    }
    ...
    // Paid-but-not-worked time is kept in its own bucket. Folding it into
    // regular would bill sick/PTO/holiday hours as worked time.
    const sick = num(ent.sick_hours) || 0;
    const vac = num(ent.vacation_hours) || 0;
    const hol = num(ent.holiday_hours) || 0;
    const other = sick + vac + hol;
    let reg = num(ent.regular_hours);
    let ot = num(ent.overtime_hours);
    // An unsplit day total counts as regular, but only the part not already
    // explained by sick/vacation/holiday.
    if (total != null && reg == null && ot == null) {
      reg = Math.max(0, round2(total - other));
      ot = 0;
    }
    byDate.set(d, {
      date: d,
      regular_hours: reg,
      overtime_hours: ot,
      sick_hours: sick || null,
      vacation_hours: vac || null,
      holiday_hours: hol || null,
      total_hours: total,
      note: ent.note || null,
      raw: ent.raw || null,
      total,
  ...
  return {
    ...
    // most-populated foreign month first, so the UI can offer one obvious switch
    offMonth: [...offMonth.entries()]
      .map(([period, count]) => ({ period, count }))
      .sort((a, b) => b.count - a.count || a.period.localeCompare(b.period)),
    statedTotal,
...
async function verifyRound(best, pdf, images, extraText, month, year, act, timed) {
  const model = verifyModelFor(best);
  const { data } = await timed("verify", model, [
...
function route(best, month, year) {
  ...
  // A document that states only a monthly/weekly total (an approval email, or a
  // weekly grid) yields no per-day entries, so the calendar — and therefore the
  // stored hours — would silently be ZERO. Make that loud instead.
  if (!best.days.length && (best.statedTotal || (best.weeklyTotals || []).length)) {
    issues.push({
      code: "NO_DAILY_BREAKDOWN", severity: "warning",
      message: best.statedTotal
        ? `This document states a total of ${best.statedTotal}h but has no day-by-day breakdown — enter your days below.`
        : "This document only shows weekly totals — enter your days below.",
    });
  }
...
function shapeEmployee(best, data, fileName, month, year) {
  ...
  return {
    ...
    issues,
    notes: best.notes,
    // Diagnostic, never a total: entries the model read that were dated OUTSIDE
    // the requested month and therefore clipped away. When `days` came back
    // empty this is the reason why, and it is the only thing that can tell the
    // employee which month to switch to.
    off_month_entries: best.offMonth || [],
    // MANAGER APPROVAL. Read from the document, gated in lib/approval.js, and
    // carried to the client UNTOUCHED. It is deliberately computed AFTER
    // route(): it is not an input to the accept gate, the review status, the
    // confidence, or any total — a signature says a human signed off, it never
    // says the hours were read correctly.
    approval: normalizeApproval(data?.manager_approval, data?.employee_name,
      best.model, { manager_name: data?.manager_name,
