...
import { SESSION_COOKIE, signSession, verifySession } from "./jwt.js";
...
export async function currentUser() {
  ...
  try {
    ...
    if (!row) return null;                       // user deleted -> session invalid
    if (row.active === false) return null;       // deactivated -> access revoked now
    // Stateless JWTs can't be individually revoked, so every token carries the
    // session_version it was minted with. Bumping that column (password reset,
    // "sign out everywhere") invalidates outstanding sessions immediately.
    if ((claims.sv ?? 1) !== row.session_version) return null;
