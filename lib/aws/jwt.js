...
// `c` is the extraction CONFIDENCE, and it is on the receipt for the same
// reason the verdict is: ts_timesheets.ai_confidence is shown to a reviewer as
// "the AI was N% sure", and an unsigned number off the wire can assert 99%
// about hours nobody extracted. It is an additive claim — receipts minted
// before it existed simply omit it and read back as null, which is the correct
// answer for a read whose confidence nobody recorded.
export async function signAiVerdict({ userId, reviewStatus, flow, confidence }) {
  const c = typeof confidence === "number" && Number.isFinite(confidence) ? confidence : null;
  return new SignJWT({ rs: reviewStatus ?? null, flow: flow ?? null, c })
    .setProtectedHeader({ alg: "HS256" })
...
// -> { reviewStatus, flow, confidence } or null.
...
// their own submission. That stays true on the on-behalf filing path — the
// receipt is checked against the ADMIN who ran the extraction, never against
// the employee the timesheet is about, who never touched /api/process.
export async function readAiVerdict(token, userId) {
  ...
  try {
    ...
    return {
      reviewStatus: payload.rs ?? null,
      flow: payload.flow ?? null,
      confidence: typeof payload.c === "number" && Number.isFinite(payload.c) ? payload.c : null,
    };
  } catch {
    return null;
  }
