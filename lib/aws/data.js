...
export async function execute(user, body) {
  try {
    ...
    if (cfg.ops && !cfg.ops.includes(op)) {
    ...
    // Payroll figures are DERIVED SERVER-SIDE from the day entries. The browser
    // sends totals too, but they are advisory only: without this, the numbers
    // that get paid are whatever the client posted.
    ...
    const cleanValues = (obj) => {
      ...
      if (table === "ts_timesheets" && Array.isArray(obj?.days)) {
        const t = deriveTotals(obj.days);
        out.monthly_regular = t.regular;
        out.monthly_overtime = t.overtime;
        out.monthly_total = t.total;
        out.days_worked = t.daysWorked;
      }
      if (table === "ts_employee_edits" && Array.isArray(obj?.days)) {
        const t = deriveTotals(obj.days);
        // fields is jsonb and already stringified below, so patch the object first
        const f = { ...(obj.fields || {}), totals: { ...(obj.fields?.totals || {}), ...t } };
